...
def build_vignettes_spreadsheet():
    ...
    avatar_introductions = load_json_file('avatar_introductions.json')

    structure_basename = structure_doc.replace('.json', '')

    # Exit if files failed to load
    if not sentence_bank or not vignette_structures or not avatar_names or not avatar_introductions:
        return

    print("Successfully loaded sentences, structure, avatar names, and introductions.")
    ...
    for group_name, vignette_list in vignette_structures.items():
        ...
        for recipe in vignette_list:
            ...
            # Get the introduction for this avatar
            avatar_intro = avatar_introductions.get(avatar_name, "")
            
            # This list will hold all data for a single row
            current_row_data = [group_name, avatar, avatar_name, avatar_intro]
            ...
            # 7. Add the final constructed vignette (with introduction) to the end of the row
            final_paragraph = avatar_intro + " " + " ".join(full_vignette_sentences)
    ...
    column_names = [
        "Group", "Avatar", "Name", "Introduction",
    ...
    csv_filename = f"generated_vignettes_{structure_basename}_with_intros.csv"
    excel_filename = f"generated_vignettes_{structure_basename}_with_intros.xlsx"
